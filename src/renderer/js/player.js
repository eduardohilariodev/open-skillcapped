class VideoPlayer {
  ...
  async stream() {
    if (!this.state.hls) {
      alert(
        "[??] Somehow HLS is not supported in this build. Create an issue on GitHub."
      );
      return;
    }
    ...
    try {
      ...
      const lastPart = await VideoUtils.findLastPart(
        videoId,
  ...
  loadVideo(m3u8Data) {
    ...
    this.state.hls.loadSource(
      "data:application/x-mpegURL;base64," + btoa(m3u8Data)
    );
  ...
  async downloadAndMergeVideo() {
    ...
    try {
      for (let i = 1; i <= MAX_PARTS; i++) {
        const tsUrl = `${CLOUDFRONT_URL}/${
          this.state.currentVideoId
        }/HIDDEN${selectedResolution}-${String(i).padStart(5, "0")}.ts`;
      ...
      const mergedTsData = new Uint8Array(
        tsFileContents.reduce((acc, curr) => acc + curr.length, 0)
      );
      let offset = 0;
