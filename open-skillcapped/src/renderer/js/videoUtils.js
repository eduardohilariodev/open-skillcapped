...
class VideoUtils {
  ...
  static async findLastPart(videoId, statusCallback) {
    ...
    try {
      for (let i = 300; i <= MAX_PARTS; i++) {
        ...
        const url = `${CLOUDFRONT_URL}/${videoId}/HIDDEN4500-${String(
          i
        ).padStart(5, "0")}.ts`;
        ...
        try {
          ...
          throw new Error(
            "Failed to fetch video, make sure the URL is correct and you are connected to the internet"
          );
        }
      }
    } finally {
  ...
  static generateM3U8(videoId, lastPart) {
    ...
    for (let i = 1; i <= lastPart; i++) {
      data += `\n#EXTINF:10,\n${CLOUDFRONT_URL}/${videoId}/HIDDEN4500-${String(
        i
      ).padStart(5, "0")}.ts`;
    }
    return data;
